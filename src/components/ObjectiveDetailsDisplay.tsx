...
export default function ObjectiveDetailsDisplay({ investment, objectiveType, objectiveYear }: Props) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      {canCalculate && objectiveType === 'revente' && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            Gain total cumulé par régime fiscal (année {objectiveYear})
          </h3>
          <div className="grid grid-cols-1 gap-4">
            {(Object.keys(REGIME_LABELS) as TaxRegime[]).map((regime) => {
              const balanceData = calculateBalanceForYear(objectiveYear, regime);
              ...
              return (
                <div
                  ...
                  className={`border rounded-lg p-4 ${
                    balanceData.totalGain >= 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                  }`}
                >
                  ...
                  <div className="text-xs text-gray-600 mt-2 space-y-1">
                    <div>Cash flow cumulé net : {formatCurrency(balanceData.cumulativeCashFlowBeforeTax)}</div>
                    <div>Solde de revente : {formatCurrency(balanceData.saleBalance)}</div>
                    <div>Impôt sur la plus-value : {formatCurrency(-balanceData.capitalGainTax)}</div>
                  </div>
                </div>
              );
            })}
      ...
      {canCalculate && objectiveType === 'cashflow' && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            Cashflow par régime fiscal (année {objectiveYear})
          </h3>
          <div className="grid grid-cols-1 gap-4">
            {(Object.keys(REGIME_LABELS) as TaxRegime[]).map((regime) => {
              const cashFlow = calculateCashFlowForRegime(objectiveYear, regime);
              
              return (
                <div
                  key={regime}
                  className={`border rounded-lg p-4 ${
                    cashFlow >= 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                  }`}
                >
                  <div className="flex justify-between items-center">
                    <span className={`text-sm font-semibold ${
                      cashFlow >= 0 ? 'text-green-900' : 'text-red-900'
                    }`}>
                      {REGIME_LABELS[regime]}
                    </span>
                    <span className={`text-xl font-bold ${
