...
export default function PropertyNavigation({ onTabChange, initialMainTab = 'acquisition', initialSubTab }: PropertyNavigationProps) {
  ...
  const getSelectedSubTabIndex = (mainTab: MainTab) => {
    switch (mainTab) {
      case 'location':
        return ['frais', 'revenus'].indexOf(selectedSubTabs[mainTab]);
      case 'imposition':
  ...
  return (
    <div className="w-full">
      ...
      {/* Sous-navigation */}
      {selectedMainTab === 'location' && (
        <Tab.Group selectedIndex={getSelectedSubTabIndex('location')} onChange={(index) => handleSubTabChange('location', ['frais', 'revenus'][index])}>
